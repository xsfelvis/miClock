...
public class XiaoMiClockView extends View {
    ...
    private XiaoMiClockAttrs mXiaoMiClockAttrs;
    ...
    public XiaoMiClockView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public XiaoMiClockView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        mXiaoMiClockAttrs = new XiaoMiClockAttrs(context, attrs, defStyleAttr);
        init();
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mSweepGradient = new SweepGradient(w / 2, h / 2,
                new int[]{mXiaoMiClockAttrs.getDarkColor(), mXiaoMiClockAttrs.getLightColor()}, new float[]{0.75f, 1});
    ...
    private void drawSecondPoint() {
        ...
        mSecondHandPaint.setColor(mXiaoMiClockAttrs.getLightColor());
        mCanvas.drawPath(mSecondHandPath, mSecondHandPaint);
        mCanvas.restore();
    }

    /**
     * 画刻度线，先画一圈梯度渲染的亮暗色渐变圆弧，重绘时不断旋转，上面盖一圈背景色的刻度线
     * 画圆的一些技巧，可以避免繁杂的角度计算
     */
     ...
     */
    private void init() {
        setBackgroundColor(mXiaoMiClockAttrs.getBackgroundColor());

        mHourHandPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mHourHandPaint.setStyle(Paint.Style.FILL);
        mHourHandPaint.setColor(mXiaoMiClockAttrs.getDarkColor());

        mMinuteHandPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mMinuteHandPaint.setColor(mXiaoMiClockAttrs.getLightColor());
        ...
        mScaleLinePaint.setColor(mXiaoMiClockAttrs.getBackgroundColor());
        ...
        mTextPaint.setColor(mXiaoMiClockAttrs.getDarkColor());
        mTextPaint.setTextSize(mXiaoMiClockAttrs.getTextSize());

        mCirclePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mCirclePaint.setStyle(Paint.Style.STROKE);
        mCirclePaint.setStrokeWidth(mCircleStrokeWidth);
        mCirclePaint.setColor(mXiaoMiClockAttrs.getDarkColor());
